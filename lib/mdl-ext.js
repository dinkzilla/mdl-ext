	    ...
	    value: function openMenu() {
	      ...
	      if (!this.isDisabled() && this.menu) {
	        ...
	        window.setTimeout(function () {
	          ...
	          _this2.scrollElements = (0, _domUtils.getScrollParents)(_this2.element);
	          _this2.scrollElements.forEach(function (el) {
	            return el.addEventListener('scroll', _this2.positionChangeHandler);
	          });
	          window.addEventListener('resize', _this2.positionChangeHandler);
	          window.addEventListener('orientationchange', _this2.positionChangeHandler);
	          _this2.menu.element.addEventListener('_closemenu', _this2.closeMenuHandler);
	
	          _this2.menu.selected = _this2.selectedItem;
	          _this2.menu.open(_this2.focusElement, position);
	          _this2.element.setAttribute('aria-expanded', 'true');
	
	          _this2.focusElementLastScrollPosition = _this2.focusElement.getBoundingClientRect();
	        }, 50); // Need a small delay (for some unknown reason). Probably need to do a focus check after opening menu.
	      }
	    }
	  }, {
	    key: 'closeMenu',
