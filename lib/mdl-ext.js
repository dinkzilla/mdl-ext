...
/***/ function(module, exports, __webpack_require__) {
	...
	(function () {
	  ...
	  var ACCORDION_ROLE = 'tablist';
	  var PRESENTATION = 'mdlext-accordion2__presentation';
	  ...
	  // Helpers
	  var selectTab_ = function selectTab_(tab) {
	    var accordion = tab.closest('.' + ACCORDION);
	
	    [].concat((0, _toConsumableArray3.default)(accordion.querySelectorAll('.' + TAB + '[aria-selected="true"]'))).forEach(function (selectedTab) {
	  ...
	  var toggleTab_ = function toggleTab_(tab) {
	    var expanded = tab.getAttribute(ARIA_EXPANDED).toLowerCase() === 'true';
	    var tabpanel = tab.nextElementSibling;
	    var presentation = tab.closest('.' + PRESENTATION);
	    ...
	    if (expanded) {
	      ...
	      var accordion = tab.closest('.' + ACCORDION);
	      var multiSelectable = accordion.getAttribute(ARIA_MULTISELECTABLE).toLowerCase() === 'true';
	
	      if (!multiSelectable) {
	        [].concat((0, _toConsumableArray3.default)(accordion.querySelectorAll('.' + TAB + '[aria-expanded="true"]'))).forEach(function (expandedTab) {
	          expandedTab.setAttribute(ARIA_EXPANDED, 'false');
	          var panel = expandedTab.nextElementSibling;
	  ...
	  MaterialExtAccordion2.prototype.upgradeTabs = function () {
	    ...
	    [].concat((0, _toConsumableArray3.default)(this.element_.querySelectorAll('.' + PRESENTATION))).forEach(function (p) {
	
	      p.setAttribute('role', PRESENTATION_ROLE);
	
	      var tab = p.querySelector('.' + TAB);
	      ...
	      if (!(0, _isInteger2.default)(tab.getAttribute('tabindex'))) {
	      ...
	      var tabpanel = p.querySelector('.' + TABPANEL);
	      var expanded = tab.getAttribute(ARIA_EXPANDED).toLowerCase() === 'true';
